...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        pe[:, 0::2] = torch.sin(
            position * div_term
        )  # even number index, (max_len, d_model/2)
        ...
        self.register_buffer(
            "pe", pe
        )  # will not be updated by back-propagation, can be called via its name
...
class ContrastLoss(nn.Module):
    def __init__(self, emsize, hidden_dim, word2idx, idx2word, device):
        ...
        #     "/home/wangshuo/weights/distilbert/"
        # )
    ...
    def map_bert(self, word_prob):
        ...
        encoded_input = self.bert_tokenizer.batch_encode_plus(
            tokens, padding=True, truncation=True, return_tensors="pt", max_length=30
        )
...
class SERMON(nn.Module):
    def __init__(
        ...
        self.predict_net = PredictionLayer(
            user_num=nuser, item_num=nitem, feature_dim=feature_dim, output=output
        )
    ...
    def forward(
        ...
        left = (
            torch.zeros(batch_size, self.ui_len + 2).bool().to(device)
        )  # (batch_size, ui_len)
        right = (
            text.t() == self.pad_idx
        ...
        src = torch.cat(
            [
                ...
                i_src.unsqueeze(0),
                w_src[0:2],
                user_retrive_glo.unsqueeze(0),
                item_retrive_glo.unsqueeze(0),
                w_src[2:],
            ],
            0,
        )  # (total_len, batch_size, emsize)
